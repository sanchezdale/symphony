...
tracker:
  ...
  project_slug: 'leftoff-app-9d940c1364f1'
  active_states:
    - Planning
    - Ready
    - In Progress
...
polling:
  interval_ms: 10000
workspace:
  root: ~/code/symphony-workspaces/leftoff
hooks:
  after_create: |
    source_repo_root="/Users/daniel/code/leftoffapp"
    source_repo_url="$SOURCE_REPO_URL"
    source_repo_base_branch="$SOURCE_REPO_BASE_BRANCH"

    if [ -z "$source_repo_url" ]; then
      source_repo_url="$(cd "$source_repo_root" && gh repo view --json sshUrl -q .sshUrl 2>/dev/null || true)"
    fi

    if [ -z "$source_repo_url" ]; then
      source_repo_url="$(git -C "$source_repo_root" remote get-url origin 2>/dev/null || true)"
    fi

    if [ -z "$source_repo_base_branch" ]; then
      source_repo_base_branch="$(cd "$source_repo_root" && gh repo view --json defaultBranchRef -q .defaultBranchRef.name 2>/dev/null || true)"
    fi

    if [ -z "$source_repo_base_branch" ]; then
      source_repo_base_branch="$(git -C "$source_repo_root" symbolic-ref --short refs/remotes/origin/HEAD 2>/dev/null | sed 's#^origin/##' || true)"
    fi

    if [ -z "$source_repo_url" ]; then
      echo "Unable to determine SOURCE_REPO_URL from gh or git for $source_repo_root." >&2
      exit 1
    fi
...
codex:
  ...
  command_by_state:
    Ready: codex --model gpt-5.3-codex-spark app-server
...
- `Planning`: do not implement yet. Refine the ticket into an execution-ready
  ...
  move the issue to `Human Review` and stop. Never start implementation from
  `Planning`.
  A human must review the proposed plan in `Human Review`, edit the task if
  needed, and manually move it to `Ready` when it is ready for implementation.
- `Todo`: default intake state. Do not start work from `Todo`; wait for a human
  to promote the issue to `Ready`.
- `Ready`: implementation-ready and queued for execution.
...
3. After updating the plan, move the issue to `Human Review`, then end the run
   without changing code or branch state.
4. If the current state is `Todo`, do not begin implementation. Wait for a
   human to move the issue to `Ready`.
...
6. Only begin or resume implementation after a human has manually moved the
   issue to `Ready`, `In Progress`, or `Rework`.
...
1. For `Ready`, `In Progress`, and `Rework`, ensure the workspace is on a branch
