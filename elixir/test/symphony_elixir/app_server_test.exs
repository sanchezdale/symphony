defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server auto-approves mcp elicitation requests with approval options" do
    test_root =
      Path.join(
        System.tmp_dir!(),
    ...
    try do
      ...
      while IFS= read -r line; do
        ...
        case \"$count\" in
          ...
          4)
            ...
            printf '%s\\n' '{\"id\":113,\"method\":\"mcpServer/elicitation/request\",\"params\":{\"questions\":[{\"header\":\"Approve app tool call?\",\"id\":\"mcp_tool_call_approval_call-720\",\"isOther\":false,\"isSecret\":false,\"options\":[{\"description\":\"Run the tool and continue.\",\"label\":\"Approve Once\"},{\"description\":\"Run the tool and remember this choice for this session.\",\"label\":\"Approve this Session\"},{\"description\":\"Decline this tool call and continue.\",\"label\":\"Deny\"}],\"question\":\"The linear MCP server wants to run the tool \\\"Save issue\\\", which may modify or delete data. Allow this action?\"}],\"server\":\"linear\"}}'
            ;;
          5)
            printf '%s\\n' '{\"method\":\"turn/completed\"}'
            exit 0
      ...
      issue = %Issue{
        ...
        description: "Ensure MCP elicitation prompts continue automatically",
        state: "In Progress",
        url: "https://example.org/issues/MT-720",
        labels: ["backend"]
      }

      assert {:ok, _result} = AppServer.run(workspace, "Handle MCP elicitation prompt", issue)
      ...
      assert Enum.any?(lines, fn line ->
               if String.starts_with?(line, "JSON:") do
                 ...
                 payload["id"] == 113 and
                   get_in(payload, ["result", "answers", "mcp_tool_call_approval_call-720", "answers"]) ==
                     ["Approve this Session"]
               else
                 false
               end
